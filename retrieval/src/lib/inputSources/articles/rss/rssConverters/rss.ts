...
const getDescription = async (article: any): Promise<string | undefined> => {
  const description =
  ...
  if (description) {
    ...
    if (/<p\b[^>]*>(.*?)<\/p>/g.test(description)) {
      return getFirstParagraphForEmbedding(description);
    }
...
export const parseRss = (feed: Feed) => (parsedXml: any) => {
